...
## EDA 인사이트
- 연령이 높을수록 심장질환 위험이 증가
- 남성이 여성보다 심장질환 위험 높음
- 흡연은 강한 위험 요인으로 작용
- 뇌졸중, 당뇨, 신장질환 등 기존 만성질환 이력은 심장질환 위험 증가와 강하게 연결
- 보행 곤란(DiffWalking) 역시 중요한 위험 신호
- 주관적 건강 상태(GenHealth)가 좋을수록 위험이 낮아지는 경향
- 일부 변수는 단변량 분석에서는 유의해 보이지만, 다변량 로지스틱 회귀에서는 독립 효과가 약해질 수 있음

## 한계
- 설문 기반 데이터의 한계로 인해 낮은 성능
- 설문 기반 데이터이므로 자기보고 편향 가능성 존재
- 미국 BRFSS 기반 데이터이므로 한국인 대상 일반화에는 한계가 있음
- 의료 진단을 대체하는 도구가 아니라 사전 위험도 확인용 보조 시스템
- 로지스틱 회귀 기반 모델이므로 복잡한 비선형 관계 반영에는 제약이 있음
